...
class DCASE2024MachineDataset(Dataset):
  ...
  def logmelspec(self, path, origin):
    ...
    # Compute the Mel spectrogram with a fixed number of Mel frequency bands and a fixed hop length
    audiomelspec = librosa.feature.melspectrogram(y=audioFile, sr=sr, n_fft=1024, hop_length=626, n_mels=64)
    audiomelspec_db = librosa.power_to_db(audiomelspec)
...
class WindowedSpectrogramDataset(Dataset):
    def __init__(self, dataset_dir, dataset, machine, extension='.npy', transform=None, isTesting=False, window_size=32, hop_size=3):
        ...
        # Precompute the number of segments for each file
        self.source_segments = self._compute_segments(self.datasetSource)
        self.target_segments = self._compute_segments(self.datasetTarget)

        # Combine source and target segments
        self.all_segments = self.source_segments + self.target_segments

    def _compute_segments(self, dataset):
        segments = []
        for idx, path in enumerate(dataset):
            ...
            if self.extension == '.npy':
                ...
                spec = self.logmelspec(path, 'getitem')  # Implement this method as needed

            # Determine the number of windows
            num_frames = spec.shape[2]
            num_windows = max(1, (num_frames - self.window_size) // self.hop_size + 1)
            ...
            for w in range(num_windows):
                ...
                if end <= num_frames:
                    ...
                    self.labels.append(self.file_labels[idx])
    ...
    def __getitem__(self, index):
        path, start, end = self.all_segments[index]
        ...
        # Extract the window
        spec_window = spec[:,:, start:end]

        # Apply transformation if specified
        if self.transform:
            spec_window = self.transform(spec_window)

        # Determine the origin
        origin = 'source' if path in self.datasetSource else 'target'

        # Assign label
        label = 1 if 'anomaly' in path else 0

        if self.isTesting:
            return spec_window, label, origin
        else:
            return spec_window, label
...
# asigur impartirea in mod egal a datelor in functie de nr de esantioane al fiecarei clase
def stratified_split(dataset, validation_size=0.25, random_state=None):
    # 'dataset.labels' contine eticheta clasei din care face masinaria parte
    labels = dataset.labels

    # creez impartirea egala pentru setul de antrenare si validare
